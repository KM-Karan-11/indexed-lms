...
// ─── Indexed LMS — Leave Management System by Indexed ─────────────────────
...
const INIT_USERS = [
  { id: "admin", name: "Admin User", email: "admin@indexed.com", password: "Admin@2024", role: "admin", manager: null, mustChangePassword: false, profile: {} },
];

function getStore() {
  try { return JSON.parse(localStorage.getItem("indexed_lms_v1") || "null"); } catch { return null; }
}
function saveStore(d) {
  try { localStorage.setItem("indexed_lms_v1", JSON.stringify(d)); } catch {}
}
...
const G = {
  ...
  dark: "linear-gradient(135deg, #1e1b4b 0%, #0B0F1A 100%)",
...
const Avatar = ({ user, size = 36 }) => {
  if (user?.profile?.photo) {
    return <img src={user.profile.photo} alt="" style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "2px solid rgba(99,102,241,0.3)" }} />;
  }
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: g, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: size * 0.35, color: "#fff", flexShrink: 0, letterSpacing: "-0.02em" }}>
      {initials}
    </div>
  );
};
...
const StatusChip = ({ status }) => {
  const map = {
    ...
    approved: { bg: "#D1FAE5", color: "#064E3B", label: "✅ Approved", dot: "#10B981" },
    rejected: { bg: "#FEE2E2", color: "#7F1D1D", label: "❌ Rejected", dot: "#EF4444" },
  };
  const s = map[status] || map.pending;
  return <span style={{ background: s.bg, color: s.color, fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 100, letterSpacing: "0.02em", whiteSpace: "nowrap" }}>{s.label}</span>;
};

const inputSt = {
  width: "100%", padding: "11px 15px", borderRadius: 12, border: `1.5px solid ${C.border}`,
  background: "#F8FAFF", fontSize: 14, color: C.text, outline: "none",
...
const Btn = ({ children, variant = "primary", size = "md", ...props }) => {
  const vs = {
    ...
    glass: { background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.25)", backdropFilter: "blur(10px)" },
    subtle: { background: C.indigoLight, color: C.indigoDark, border: "none" },
  };
  const sz = { sm: "7px 14px", md: "11px 22px", lg: "14px 32px" };
  return (
    <button {...props} style={{ padding: sz[size], borderRadius: 12, fontWeight: 700, fontSize: size === "sm" ? 12 : 14, cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s", letterSpacing: "-0.01em", ...vs[variant], ...(props.disabled ? { opacity: 0.5, cursor: "not-allowed" } : {}), ...props.style }}>
      {children}
    </button>
  );
};
...
const GlassCard = ({ children, style, gradient }) => (
  <div style={{ background: gradient ? `${gradient}, rgba(255,255,255,0.9)` : C.card, borderRadius: 20, border: `1px solid ${C.border}`, padding: 24, backdropFilter: "blur(20px)", ...style }}>
    {children}
  </div>
);
...
const Toast = ({ toast }) => {
  ...
  return (
    <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 9999, background: toast.type === "error" ? C.danger : C.dark, color: "#fff", padding: "14px 22px", borderRadius: 16, fontWeight: 700, fontSize: 14, maxWidth: 360, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", display: "flex", alignItems: "center", gap: 10, animation: "slideUp 0.3s ease" }}>
  ...
  return (
    <div style={{ width: 240, minHeight: "100vh", background: C.dark, display: "flex", flexDirection: "column", flexShrink: 0, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ padding: "16px 12px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <button onClick={logout} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, border: "none", background: "transparent", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 500 }}>
          <span>🚪</span> Sign out
        </button>
      </div>
    </div>
...
export default function App() {
  const [store, setStore] = useState(() => getStore() || { users: INIT_USERS, requests: [], slackWebhook: "" });
  ...
  const login = () => {
    const user = store.users.find(u => u.email === loginEmail && u.password === loginPass);
    if (!user) return setLoginError("Wrong email or password. Try again 👀");
    setLoginError("");
    setSession(user);
    setPage(user.mustChangePassword ? "changepass" : "dashboard");
  };

  const logout = () => { setSession(null); setPage("login"); setLoginEmail(""); setLoginPass(""); };
  ...
  if (page === "changepass") return <ChangePassPage session={session} store={store} updateStore={updateStore} setSession={setSession} setPage={setPage} notify={notify} />;

  const liveSession = store.users.find(u => u.id === session.id) || session;
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus, select:focus, textarea:focus { border-color: ${C.indigo} !important; box-shadow: 0 0 0 3px rgba(99,102,241,0.12); }
      ...
      <main style={{ flex: 1, padding: "32px 36px", overflow: "auto", background: `${G.mesh}, ${C.bg}` }}>
        <div className="page-content" key={page}>
          {page === "dashboard" && <DashboardPage session={liveSession} store={store} nav={nav} />}
          {page === "request" && <RequestPage session={liveSession} store={store} updateStore={updateStore} notify={notify} />}
          {page === "approvals" && <ApprovalsPage session={liveSession} store={store} updateStore={updateStore} notify={notify} />}
          {page === "all_requests" && liveSession.role === "admin" && <AllRequestsPage store={store} updateStore={updateStore} notify={notify} />}
          {page === "admin" && liveSession.role === "admin" && <AdminPage store={store} updateStore={updateStore} notify={notify} />}
          {page === "profile" && <ProfilePage session={liveSession} store={store} updateStore={updateStore} setSession={setSession} notify={notify} />}
        </div>
      </main>
    </div>
  );
...
function LoginPage({ email, setEmail, pass, setPass, error, onLogin }) {
  return (
    <div style={{ minHeight: "100vh", background: C.dark, display: "flex", fontFamily: "'Inter', system-ui, sans-serif", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 60, background: "radial-gradient(ellipse at 30% 40%, rgba(99,102,241,0.25) 0%, transparent 60%), radial-gradient(ellipse at 80% 70%, rgba(139,92,246,0.2) 0%, transparent 50%)" }}>
        <div style={{ textAlign: "center", animation: "fadeIn 0.6s ease" }}>
          <div style={{ width: 80, height: 80, borderRadius: 24, background: G.indigo, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 28px", boxShadow: "0 20px 60px rgba(99,102,241,0.5)", animation: "float 4s ease-in-out infinite" }}>
          ...
          {/* Feature pills */}
          {[["✈️", "Request leave in seconds"], ["📋", "Real-time approvals"], ["💬", "Slack notifications"], ["👥", "Team directory"]].map(([icon, text]) => (
            <div key={text} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 100, padding: "8px 16px", margin: "4px", fontSize: 13, color: "rgba(255,255,255,0.7)", fontWeight: 500 }}>
              <span>{icon}</span>{text}
            </div>
          ))}
        </div>
      ...
      <div style={{ width: 440, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: 48 }}>
        <div style={{ width: "100%", animation: "fadeIn 0.5s ease 0.1s both" }}>
          ...
          {error && (
            <div style={{ background: C.dangerLight, border: `1px solid ${C.danger}33`, borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: C.danger, fontWeight: 600 }}>{error}</div>
          )}
          <button onClick={onLogin} style={{ width: "100%", padding: "14px", borderRadius: 14, background: G.indigo, color: "#fff", border: "none", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 8px 25px rgba(99,102,241,0.4)", letterSpacing: "-0.02em", marginBottom: 16 }}>
            Sign in →
          </button>
        </div>
      </div>
...
function ChangePassPage({ session, store, updateStore, setSession, setPage, notify }) {
  ...
  const save = () => {
    ...
    updateStore(s => { const u = s.users.find(u => u.id === session.id); if (u) { u.password = nxt; u.mustChangePassword = false; } });
    setSession({ ...session, password: nxt, mustChangePassword: false });
...
function DashboardPage({ session, store, nav }) {
  const reqs = store.requests.filter(r => r.userId === session.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const counts = { total: reqs.length, approved: reqs.filter(r => r.status === "approved").length, pending: reqs.filter(r => r.status === "pending").length, rejected: reqs.filter(r => r.status === "rejected").length };

  const hour = new Date().getHours();
  const greeting = hour < 12 ? "Good morning" : hour < 17 ? "Good afternoon" : "Good evening";
  const greetEmoji = hour < 12 ? "☀️" : hour < 17 ? "👋" : "🌙";

  const upcoming = reqs.filter(r => r.status === "approved" && new Date(r.endDate) >= new Date()).slice(0, 2);
  const pendingApprovals = (session.role === "manager" || session.role === "admin") ? store.requests.filter(r => r.managerId === session.id && r.status === "pending") : [];
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 18, position: "relative" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", margin: 0 }}>
              {session.profile?.jobTitle || session.role} {session.profile?.department ? `· ${session.profile.department}` : ""}
            </p>
          </div>
          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <button onClick={() => nav("request")} style={{ background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", padding: "10px 20px", borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", backdropFilter: "blur(10px)" }}>
              ✈️ Request Leave
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
        {[
          { label: "Total Requests", val: counts.total, icon: "📊", g: G.indigo, sub: "all time" },
          { label: "Approved", val: counts.approved, icon: "✅", g: G.emerald, sub: "successful" },
          { label: "Pending", val: counts.pending, icon: "⏳", g: G.amber, sub: "awaiting review" },
          { label: "Rejected", val: counts.rejected, icon: "❌", g: G.pink, sub: "declined" },
        ].map((s, i) => (
          <div key={s.label} style={{ background: C.card, borderRadius: 18, padding: "20px", border: `1px solid ${C.border}`, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <GlassCard>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <span style={{ fontSize: 12, color: C.muted, fontWeight: 600 }}>{reqs.length} total</span>
          </div>
          {reqs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "28px 0" }}>
              <div style={{ fontSize: 36, marginBottom: 10 }}>🏖️</div>
              <p style={{ color: C.muted, fontSize: 14, margin: 0, fontWeight: 600 }}>No requests yet</p>
              <p style={{ color: C.muted, fontSize: 12, margin: "4px 0 0" }}>Time to plan a vacay?</p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {reqs.slice(0, 4).map(r => {
                ...
                return (
                  <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 12, background: C.bg }}>
                    <span style={{ fontSize: 22 }}>{LEAVE_EMOJI[r.type] || "📋"}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{r.type}</div>
                      <div style={{ fontSize: 11, color: C.muted, fontWeight: 500 }}>{r.startDate} · {days}d</div>
                    </div>
        ...
        {/* Pending approvals or upcoming leaves */}
        <GlassCard>
          {pendingApprovals.length > 0 ? (
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {pendingApprovals.slice(0, 4).map(r => {
                  const emp = store.users.find(u => u.id === r.userId);
                  const days = Math.ceil((new Date(r.endDate) - new Date(r.startDate)) / 86400000) + 1;
                  return (
                    <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, background: C.bg }}>
                      <Avatar user={emp} size={30} />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{emp?.name?.split(" ")[0]}</div>
                        <div style={{ fontSize: 11, color: C.muted }}>{r.type} · {days}d</div>
                      </div>
                      <StatusChip status={r.status} />
                    </div>
                  );
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[["✈️", "Take some time off", "request", C.indigo], ["👤", "Update your profile", "profile", C.violet], ["📊", "View all requests", "all_requests", C.cyan]].filter(([,, id]) => id !== "all_requests" || session.role === "admin").map(([icon, text, id, color]) => (
                  <button key={id} onClick={() => nav(id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, background: C.bg, border: "none", cursor: "pointer", fontFamily: "inherit", textAlign: "left", transition: "all 0.15s" }}>
...
function RequestPage({ session, store, updateStore, notify }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const req = { id: Date.now().toString(), userId: session.id, type, startDate: start, endDate: end, reason, status: "pending", createdAt: new Date().toISOString(), managerId: session.manager };
    updateStore(s => s.requests.push(req));
    if (store.slackWebhook) {
      fetch(store.slackWebhook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: `${LEAVE_EMOJI[type]} *New Leave Request* from *${session.name}*\n• Type: ${type}\n• Dates: ${start} → ${end} (${days} day${days !== 1 ? "s" : ""})\n• Reason: ${reason || "—"}\n• Manager: ${manager?.name || "Unassigned"}` }) }).catch(() => {});
    }
  ...
  return (
    <div style={{ maxWidth: 560 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {LEAVE_TYPES.map(t => (
            <button key={t} onClick={() => setType(t)} style={{ padding: "14px 16px", borderRadius: 14, border: `2px solid ${type === t ? C.indigo : C.border}`, background: type === t ? C.indigoLight : C.bg, color: type === t ? C.indigoDark : C.muted, fontWeight: type === t ? 800 : 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8, transition: "all 0.15s" }}>
              <span style={{ fontSize: 20 }}>{LEAVE_EMOJI[t]}</span>
              <span>{t}</span>
            </button>
          ))}
        </div>
      ...
      <GlassCard style={{ marginBottom: 16 }}>
        ...
        {days > 0 && (
          <div style={{ background: C.indigoLight, borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>📊</span>
            <span style={{ fontSize: 14, fontWeight: 700, color: C.indigoDark }}>{days} day{days !== 1 ? "s" : ""} of leave</span>
          </div>
        )}
      </GlassCard>
      ...
      <button onClick={submit} disabled={busy} style={{ width: "100%", padding: "15px", borderRadius: 16, background: busy ? "#ccc" : G.indigo, color: "#fff", border: "none", fontSize: 16, fontWeight: 800, cursor: busy ? "not-allowed" : "pointer", fontFamily: "inherit", boxShadow: busy ? "none" : "0 8px 25px rgba(99,102,241,0.4)", letterSpacing: "-0.02em" }}>
...
function ApprovalsPage({ session, store, updateStore, notify }) {
  const [filter, setFilter] = useState("all");
  const reqs = store.requests.filter(r => r.managerId === session.id && (filter === "all" || r.status === filter)).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  const decide = async (id, decision) => {
    updateStore(s => { const r = s.requests.find(r => r.id === id); if (r) r.status = decision; });
    const r = store.requests.find(r => r.id === id);
    const emp = store.users.find(u => u.id === r?.userId);
    if (store.slackWebhook && emp) {
      fetch(store.slackWebhook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: `${decision === "approved" ? "✅" : "❌"} *Leave ${decision}* — ${emp.name}\n• ${r.type} · ${r.startDate} → ${r.endDate}\n• Reviewed by ${session.name}` }) }).catch(() => {});
    }
    notify(decision === "approved" ? `Approved! ${emp?.name?.split(" ")[0]} will be stoked 🎉` : `Declined. Better luck next time. 🤷`, decision === "approved" ? "success" : "error");
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h2 style={{ fontSize: 26, fontWeight: 900, color: C.text, margin: "0 0 4px", letterSpacing: "-0.04em" }}>Approvals 📋</h2>
          <p style={{ color: C.muted, fontSize: 14, margin: 0, fontWeight: 500 }}>Your team's leave requests</p>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {["all", "pending", "approved", "rejected"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: "7px 14px", borderRadius: 10, border: `1.5px solid ${filter === f ? C.indigo : C.border}`, background: filter === f ? C.indigoLight : "transparent", color: filter === f ? C.indigoDark : C.muted, fontWeight: filter === f ? 700 : 500, fontSize: 12, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
              {f}
            </button>
          ))}
        </div>
      ...
      {reqs.length === 0 ? (
        <GlassCard style={{ textAlign: "center", padding: "60px 24px" }}>
          ...
          <p style={{ fontWeight: 800, fontSize: 16, color: C.text, margin: "0 0 6px" }}>All clear!</p>
          <p style={{ color: C.muted, fontSize: 14, margin: 0 }}>No requests to review right now.</p>
        </GlassCard>
      ) : (
...
function AllRequestsPage({ store, updateStore, notify }) {
  const [filter, setFilter] = useState("all");
  ...
  const reqs = store.requests
    ...
    .filter(r => {
      ...
      const emp = store.users.find(u => u.id === r.userId);
      return emp?.name.toLowerCase().includes(search.toLowerCase()) || r.type.toLowerCase().includes(search.toLowerCase());
    })
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontSize: 26, fontWeight: 900, color: C.text, margin: "0 0 4px", letterSpacing: "-0.04em" }}>All Requests 🗂️</h2>
        <p style={{ color: C.muted, fontSize: 14, margin: 0, fontWeight: 500 }}>Full team leave overview</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["all", "pending", "approved", "rejected"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: "9px 16px", borderRadius: 10, border: `1.5px solid ${filter === f ? C.indigo : C.border}`, background: filter === f ? C.indigoLight : "transparent", color: filter === f ? C.indigoDark : C.muted, fontWeight: filter === f ? 700 : 500, fontSize: 12, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
              {f}
            </button>
          ))}
        </div>
      ...
      {reqs.length === 0 ? (
        <GlassCard style={{ textAlign: "center", padding: "50px 24px" }}>
          ...
          <p style={{ fontWeight: 700, color: C.text, margin: 0 }}>No requests found</p>
        </GlassCard>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {reqs.map(r => {
            const emp = store.users.find(u => u.id === r.userId);
            const mgr = store.users.find(u => u.id === r.managerId);
            const days = Math.ceil((new Date(r.endDate) - new Date(r.startDate)) / 86400000) + 1;
            return (
              <GlassCard key={r.id} style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", padding: "16px 20px" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {mgr && <span style={{ fontSize: 11, color: C.muted, fontWeight: 500 }}>→ {mgr.name}</span>}
                  </div>
...
function AdminPage({ store, updateStore, notify }) {
  const [tab, setTab] = useState("users");
  const [f, setF] = useState({ name: "", email: "", role: "member", manager: "" });
  const [slack, setSlack] = useState(store.slackWebhook || "");
  const managers = store.users.filter(u => u.role === "manager" || u.role === "admin");

  const addUser = () => {
    if (!f.name || !f.email) return notify("Name + email required 🙏", "error");
    if (store.users.find(u => u.email === f.email)) return notify("Email already exists 👀", "error");
    const pass = "Welcome@" + Math.floor(1000 + Math.random() * 9000);
    updateStore(s => s.users.push({ id: Date.now().toString(), name: f.name, email: f.email, password: pass, role: f.role, manager: f.manager || null, mustChangePassword: true, profile: {} }));
    notify(`${f.name} added! 🎉 Temp password: ${pass}`);
    setF({ name: "", email: "", role: "member", manager: "" });
  };

  const removeUser = (id) => {
    if (id === "admin") return notify("Can't remove the primary admin 🚫", "error");
    updateStore(s => { s.users = s.users.filter(u => u.id !== id); });
    notify("Member removed.");
  };

  const tabs = [["users", "👥", "Team"], ["add", "➕", "Add Member"], ["slack", "💬", "Slack"]];

  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h2 style={{ fontSize: 26, fontWeight: 900, color: C.text, margin: "0 0 4px", letterSpacing: "-0.04em" }}>Admin ⚙️</h2>
        <p style={{ color: C.muted, fontSize: 14, margin: 0, fontWeight: 500 }}>Manage your team and integrations</p>
      </div>

      {/* Stat bar */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Team Members", val: store.users.length, icon: "👥", g: G.indigo },
          { label: "Total Requests", val: store.requests.length, icon: "📊", g: G.cyan },
          { label: "Pending Review", val: store.requests.filter(r => r.status === "pending").length, icon: "⏳", g: G.amber },
        ].map(s => (
          <div key={s.label} style={{ background: C.card, borderRadius: 16, padding: "18px 20px", border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <div style={{ fontSize: 24, fontWeight: 900, color: C.text, letterSpacing: "-0.04em" }}>{s.val}</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {tabs.map(([key, icon, label]) => (
          <button key={key} onClick={() => setTab(key)} style={{ padding: "10px 20px", borderRadius: 12, border: `2px solid ${tab === key ? C.indigo : C.border}`, background: tab === key ? C.indigoLight : "transparent", color: tab === key ? C.indigoDark : C.muted, fontWeight: tab === key ? 800 : 500, fontSize: 13, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
            {icon} {label}
          </button>
        ))}
      </div>
          ...
          {store.users.map(u => (
            <GlassCard key={u.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 20px" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2, fontWeight: 500 }}>{u.email} {u.profile?.department ? `· ${u.profile.department}` : ""}</div>
                {u.manager && <div style={{ fontSize: 11, color: C.muted, marginTop: 1 }}>Reports to {store.users.find(m => m.id === u.manager)?.name}</div>}
              </div>
              ...
              {u.id !== "admin" && (
                <button onClick={() => removeUser(u.id)} style={{ padding: "7px 14px", borderRadius: 10, background: C.dangerLight, color: C.danger, border: `1.5px solid ${C.danger}22`, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Remove</button>
              )}
            </GlassCard>
          ))}
        </div>
      ...
      {tab === "add" && (
        <GlassCard style={{ maxWidth: 500 }}>
          ...
          <div style={{ background: "#FEF3C7", border: "1px solid #F59E0B33", borderRadius: 12, padding: "12px 16px", marginBottom: 18, fontSize: 13, color: "#92400E", fontWeight: 600 }}>
            ⚡ A temp password is auto-generated. They'll be prompted to change it on first login.
          </div>
          <button onClick={addUser} style={{ width: "100%", padding: 14, borderRadius: 14, background: G.indigo, color: "#fff", border: "none", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 8px 25px rgba(99,102,241,0.35)" }}>
            Add Member 🚀
          </button>
        </GlassCard>
      )}
      ...
      {tab === "slack" && (
        <GlassCard style={{ maxWidth: 540 }}>
          ...
          <p style={{ fontSize: 13, color: C.muted, marginBottom: 18, fontWeight: 500 }}>Get pinged in Slack when leaves are submitted or reviewed.</p>
          <div style={{ background: C.indigoLight, border: `1px solid ${C.indigo}22`, borderRadius: 14, padding: "14px 18px", marginBottom: 18 }}>
            <p style={{ fontSize: 12, fontWeight: 800, color: C.indigoDark, margin: "0 0 8px" }}>🛠 How to set up:</p>
            <ol style={{ fontSize: 12, color: C.indigoDark, margin: 0, paddingLeft: 18, lineHeight: 2 }}>
              <li>api.slack.com/apps → Create New App → From scratch</li>
              <li>Incoming Webhooks → Activate → Add to workspace</li>
          ...
          <button onClick={() => { updateStore(s => s.slackWebhook = slack); notify("Slack connected! 🎉"); }} style={{ width: "100%", padding: 14, borderRadius: 14, background: G.indigo, color: "#fff", border: "none", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>
            Save Webhook 💬
          </button>
          {store.slackWebhook && (
            <div style={{ marginTop: 16, background: "#D1FAE5", borderRadius: 12, padding: "10px 16px", fontSize: 13, fontWeight: 700, color: "#064E3B", textAlign: "center" }}>
              ✅ Slack is connected and ready!
            </div>
          )}
        </GlassCard>
      )}
...
function ProfilePage({ session, store, updateStore, setSession, notify }) {
  ...
  const save = () => {
    ...
    updateStore(s => { const u = s.users.find(u => u.id === session.id); if (u) { u.name = fullName; u.profile = { ...p }; } });
    setSession(prev => ({ ...prev, name: fullName, profile: { ...p } }));
  ...
  const changePw = () => {
    ...
    updateStore(s => { const u = s.users.find(u => u.id === session.id); if (u) u.password = nxt; });
    setSession(prev => ({ ...prev, password: nxt }));
  ...
  const myRequests = store.requests.filter(r => r.userId === session.id);
  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ background: G.indigo, borderRadius: 24, padding: "36px", marginBottom: 16, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 20, position: "relative", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.65)", margin: "0 0 12px", fontWeight: 500 }}>{p.jobTitle || session.role} {p.department ? `· ${p.department}` : ""}</p>
          ...
          <button onClick={() => setPwOpen(!pwOpen)} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.25)", color: "#fff", padding: "8px 16px", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      {pwOpen && (
        <GlassCard style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Current</label><input type="password" value={cur} onChange={e => setCur(e.target.value)} style={inputSt} /></div>
            <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>New</label><input type="password" value={nxt} onChange={e => setNxt(e.target.value)} style={inputSt} /></div>
            <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>Confirm</label><input type="password" value={conf} onChange={e => setConf(e.target.value)} style={inputSt} /></div>
          </div>
      ...
      <GlassCard style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          {[["firstName", "First Name", "Jane", "text"], ["lastName", "Last Name", "Smith", "text"], ["birthdate", "Date of Birth", "", "date"], ["mobile", "Mobile Number", "+1 555 000 0000", "text"], ["city", "City", "London", "text"], ["country", "Country", "United Kingdom", "text"]].map(([key, label, ph, type]) => (
            <div key={key}>
              <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.07em" }}>{label}</label>
              <input type={type} value={p[key]} onChange={e => setP(prev => ({ ...prev, [key]: e.target.value }))} placeholder={ph} style={inputSt} />
            </div>
          ))}
        </div>
        <div style={{ marginTop: 14 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.07em" }}>Bio</label>
          <textarea value={p.bio} onChange={e => setP(prev => ({ ...prev, bio: e.target.value }))} rows={2} placeholder="A little about you… 🌟" style={{ ...inputSt, resize: "vertical" }} />
        </div>
      </GlassCard>
      ...
      <GlassCard style={{ marginBottom: 24 }}>
        <p style={{ fontSize: 14, fontWeight: 800, color: C.text, margin: "0 0 4px", paddingBottom: 0 }}>🆘 Emergency Contact</p>
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 16, fontWeight: 500 }}>Only visible to admins and managers.</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.07em" }}>Name</label><input value={p.emergencyName} onChange={e => setP(prev => ({ ...prev, emergencyName: e.target.value }))} placeholder="John Smith" style={inputSt} /></div>
          <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.07em" }}>Relationship</label><input value={p.emergencyRelation} onChange={e => setP(prev => ({ ...prev, emergencyRelation: e.target.value }))} placeholder="Spouse" style={inputSt} /></div>
          <div><label style={{ fontSize: 11, fontWeight: 700, color: C.muted, display: "block", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.07em" }}>Phone</label><input value={p.emergencyPhone} onChange={e => setP(prev => ({ ...prev, emergencyPhone: e.target.value }))} placeholder="+1 555 000 0001" style={inputSt} /></div>
        </div>
      </GlassCard>

      <button onClick={save} style={{ padding: "14px 36px", borderRadius: 16, background: G.indigo, color: "#fff", border: "none", fontSize: 16, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 8px 25px rgba(99,102,241,0.4)", letterSpacing: "-0.02em" }}>
        Save Profile 🔥
      </button>
    </div>
  );
