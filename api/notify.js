...
// ─── Build Slack approval message with buttons ────────────────────────────
  ...
  try {
    ...
    if (type === "decision") {
      ...
      await sendSlackDM(employeeEmail, `${emoji} Your leave has been ${decision} by ${managerName}`, [
        ...
        { type: "section", text: { type: "mrkdwn", text: approved ? `Your Slack status has been set to OOO 🌴\n<${APP_URL}|View on Indexed LMS>` : `Please speak to your manager.\n<${APP_URL}|View on Indexed LMS>` } },
      ]);
    ...
    if (type === "invite") {
      ...
      await sendEmail(userEmail, `🌴 Welcome to Indexed LMS!`,
        emailBase(`<h2>Welcome! 🎉</h2><p>Hi <strong>${userName}</strong>, you've been added to Indexed LMS.</p><div class="info-box"><div class="info-row"><span class="info-label">URL</span><span class="info-val"><a href="${APP_URL}" style="color:#6366F1">${APP_URL}</a></span></div><div class="info-row"><span class="info-label">Email</span><span class="info-val">${userEmail}</span></div><div class="info-row"><span class="info-label">Temp Password</span><span class="info-val" style="font-family:monospace">${tempPassword}</span></div></div><p>⚡ Set a new password on first login.</p><p style="text-align:center"><a href="${APP_URL}" class="btn" style="background:linear-gradient(135deg,#6366F1,#8B5CF6);color:#fff">Sign in →</a></p>`)
      );
